...
namespace Phoenix::Blackboard
{
    ...
    template <>
    struct BlackboardValueConverter<FName>
    {
        static FName ConvertFrom(blackboard_value_t value)
        {
            return FName(static_cast<hash32_t>(value));
        }
        static blackboard_value_t ConvertTo(const FName& value)
        {
            return static_cast<hash32_t>(value);
        }
    };
    ...
    struct BlackboardComplexValueAccessor<TBlackboard, Vec2>
    {
        ...
        {
            auto xKey = BlackboardKey::AppendKeyLo(key, XName);
            auto yKey = BlackboardKey::AppendKeyLo(key, YName);
    ...
    {
        static constexpr char RName[] = "R";
        static constexpr char GName[] = "G";
        static constexpr char BName[] = "B";
        ...
        static bool HasValue(const TBlackboard& set, const BlackboardQuery& query)
        {
            ...
            auto gQuery = colorQuery.AppendKeyLo(GName);
            auto bQuery = colorQuery.AppendKeyLo(BName);
        ...
        static bool GetValue(const TBlackboard& set, const BlackboardQuery& query, Color& outValue)
        {
            ...
            auto gQuery = colorQuery.AppendKeyLo(GName);
            auto bQuery = colorQuery.AppendKeyLo(BName);
            auto aQuery = colorQuery.AppendKeyLo(AName);
        ...
        static bool SetValue(TBlackboard& set, blackboard_key_t key, const Color& value)
        {
            ...
            auto rKey = BlackboardKey::AppendKeyLo(colorKey, RName);
            auto gKey = BlackboardKey::AppendKeyLo(colorKey, GName);
            auto bKey = BlackboardKey::AppendKeyLo(colorKey, BName);
            ...
            if (value.A != 255)
            {
                auto aKey = BlackboardKey::AppendKeyLo(colorKey, AName);
