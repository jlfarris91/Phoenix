...
namespace Phoenix::LDS
{
    ...
    TObjectRefPtr LDSObjectRefArrayPtr::ItemAs(uint32 index) const
    {
        return LDSArrayUtil::GetItem<TObjectRefPtr>(Path, index, Flags);
    }
    ...
    const LDSObjectRefArrayPtr& LDSObjectRefArrayPtr::ForEachItemAs(
        const ILDSQueryContext& context,
        const TCallback& callback) const
    {
        LDSArrayUtil::ForEachItem<TObjectRefPtr>(context, Path, callback, Flags);
    ...
    uint32 LDSObjectRefArrayPtr::GetItemsAs(
        const ILDSQueryContext& context,
        TContainer& outItems) const
    {
        return LDSArrayUtil::GetItems<TObjectRefPtr>(context, Path, outItems, Flags);
    ...
    template <class TCallback>
    const TLDSObjectRefArrayPtrBase<TObjectPtr, TObjectRefPtr>&
        TLDSObjectRefArrayPtrBase<TObjectPtr, TObjectRefPtr>::ForEachResolvedReadItemObject(
            const ILDSQueryContext& context,
            const TCallback& callback) const
    {
        LDSArrayUtil::ForEachItemAsResolvedReadObject<typename TObjectPtr::ObjectT, TObjectPtr, TObjectRefPtr>(context, Path, callback, Flags);
        return *this;
    }
