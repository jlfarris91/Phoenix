...
namespace Phoenix
{
    ...
    PHOENIX_SIM_API constexpr uint32 CollapseBits(uint64 v)
    {
        uint64 x = v;
    ...
    PHOENIX_SIM_API constexpr uint64 ToMortonCode(uint32 x, uint32 y, uint8 lshift = MortonCodeGridBits)
    {
    ...
    PHOENIX_SIM_API constexpr uint8 GetMortonCodeQuad(uint64 zcode)
    {
        return zcode >> 61;
    }

    PHOENIX_SIM_API constexpr uint64 GetMortonCodeValue(uint64 zcode)
    {
    ...
    // Quadrant 2: -x, -y
    // Quadrant 3: +x, -y
    PHOENIX_SIM_API constexpr uint64 ToMortonCode(int32 x, int32 y, uint8 lshift = MortonCodeGridBits)
    {
        uint64 quad = 0;
        if (x < 0) quad |= 1;
        if (y < 0) quad |= 2;
    ...
    PHOENIX_SIM_API constexpr uint64 ToMortonCode(Distance x, Distance y)
    {
        return ToMortonCode(int32(x), int32(y), MortonCodeGridBits);
    }

    PHOENIX_SIM_API constexpr uint64 ToMortonCode(const Vec2& v)
    {
        return ToMortonCode(v.X, v.Y);
    }

    PHOENIX_SIM_API constexpr void FromMortonCode(uint64 zcode, uint32& outX, uint32& outY, uint8 rshift = MortonCodeGridBits)
    {
        outX = CollapseBits(zcode >> 1) << rshift;
        outY = CollapseBits(zcode) << rshift;
    }

    PHOENIX_SIM_API constexpr void FromMortonCode(uint64 zcode, int32& outX, int32& outY, uint8 rshift = MortonCodeGridBits)
    {
    ...
    PHOENIX_SIM_API constexpr void FromMortonCode(uint64 zcode, Distance& outX, Distance& outY, uint8 rshift = MortonCodeGridBits)
    {
        int32 x, y;
    ...
    PHOENIX_SIM_API constexpr Vec2 FromMortonCode(uint64 zcode, uint8 rshift = MortonCodeGridBits)
    {
        int32 x, y;
        FromMortonCode(zcode, x, y, rshift);
        return { x, y };
    ...
    PHOENIX_SIM_API constexpr int32 FromMortonCodeX(uint64 zcode, uint8 rshift = MortonCodeGridBits)
    {
        uint8 quad = GetMortonCodeQuad(zcode);
        uint64 value = GetMortonCodeValue(zcode);
    ...
    PHOENIX_SIM_API constexpr Distance FromMortonCodeX_Dist(uint64 zcode)
    {
        return FromMortonCodeX(zcode);
    }

    PHOENIX_SIM_API constexpr int32 FromMortonCodeY(uint64 zcode, uint8 rshift = MortonCodeGridBits)
    {
        uint8 quad = GetMortonCodeQuad(zcode);
        uint64 value = GetMortonCodeValue(zcode);
    ...
    PHOENIX_SIM_API constexpr Distance FromMortonCodeY_Dist(uint64 zcode)
    {
        return FromMortonCodeY(zcode);
    }

    // Scales down to morton code space and preserves sign.
    PHOENIX_SIM_API constexpr int32 ScaleToMortonCode(int32 x)
    {
        int8 sign = x >= 0 ? 1 : -1;
        uint32 xu = x < 0 ? -x - 1 : x;
    ...
    struct PHOENIX_SIM_API MortonCodeAABB
    {
        int32 MinX = 0, MinY = 0;
        int32 MaxX = 0, MaxY = 0;
    };

    PHOENIX_SIM_API MortonCodeAABB ToMortonCodeAABB(const Vec2& min, const Vec2& max);
    PHOENIX_SIM_API MortonCodeAABB ToMortonCodeAABB(const Vec2& pos, Distance radius);

    PHOENIX_SIM_API void MortonCodeQuery(
