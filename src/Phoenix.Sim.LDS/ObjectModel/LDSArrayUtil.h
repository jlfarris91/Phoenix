...
{
    struct PHOENIX_SIM_API LDSArrayUtil final
    {
        ...
        // Enumerates the object reference items of an array, resolves them to their object pointers, reads the objects,
        // and calls a callback for each read object.
        template <IsNotRecordPtr TObject, IsObjectPtr TObjectPtr, IsObjectRefPtr TObjectRefPtr, class TCallback>
        static void ForEachItemAsResolvedReadObject(
            const ILDSQueryContext& context,
            const LDSRecordPath& path,
            const TCallback& callback,
            ELDSRecordQueryFlags flags = ELDSRecordQueryFlags::None);
