...
std::shared_ptr<Phoenix::Scene::IScene> Phoenix::EnTT::SceneManager::CreateScene(FName id)
{
...
std::shared_ptr<Phoenix::Scene::IScene> Phoenix::EnTT::SceneManager::FindScene(FName id)
{
    auto iter = Scenes.find(id);
    return iter != Scenes.end() ? iter->second : nullptr;
}

bool Phoenix::EnTT::SceneManager::DestroyScene(FName id)
{
    auto iter = Scenes.find(id);
    if (iter == Scenes.end())
    {
...
std::vector<std::shared_ptr<Phoenix::Scene::IScene>> Phoenix::EnTT::SceneManager::GetAllScenes()
{
