...
> **Status (2026-03)**: The sol2-based approach described in this document has been superseded.
> `FeatureLua` no longer owns a `sol::state` — it delegates WASM runtime management to
> `FeatureScript` and loads Lua scripts into the `LuaRunner.wasm` interpreter at runtime.
> The `IScriptRuntime` / `IScriptBindings` interfaces still exist for the host-function bridge
> (generated by `PhoenixWasmGen` via TypeRegistry), but the sol2 adapter (`LuaRuntime`) is
> no longer wired into `FeatureLua`.
> See [WasmScriptingInternals.md](WasmScriptingInternals.md) for the current architecture.

This document describes the original design intent for a runtime-agnostic scripting binding
system. It remains useful background for the `IScriptRuntime` / `IScriptBindings` interfaces
and the TypeRegistry-driven host-function registration pattern.
