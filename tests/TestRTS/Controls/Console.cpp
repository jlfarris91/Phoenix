...
void Console::Init(const std::shared_ptr<Phoenix::ILogger>& logger)
{
    Logger = logger;
    std::static_pointer_cast<::Logger>(logger)->OnLog([this](Phoenix::ELogLevel level, const std::string& msg)
    {
...
void Console::Draw()
{
    ...
    if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve), ImGuiChildFlags_NavFlattened, ImGuiWindowFlags_HorizontalScrollbar))
    {
        ...
        for (uint32_t i = ItemsReadIdx; i != ItemsWriteIdx; i = (i + 1) % _countof(Items))
        {
            ...
            if (strstr(item, "[Verbose]")) { color = ImVec4(0.5f, 0.5f, 0.5f, 1.0f); has_color = true; }
            if (strstr(item, "[Warn]")) { color = ImVec4(1.0f, 0.9f, 0.2f, 1.0f); has_color = true; }
